    ...
    >
          ...
          <div className="relative">
            <div
              className={`w-12 h-12 rounded-2xl flex items-center justify-center text-xl ${
                isDeployed ? 'bg-emerald-500/20' : 'bg-slate-800'
              }`}
            >
              {isDeployed ? '🟢' : '⚪'}
            </div>
            {isDeployed && (
              <div className="absolute inset-0 rounded-2xl bg-emerald-400/20 animate-ping" style={{ animationDuration: '2.5s' }} />
            )}
          </div>
          <div>
        ...
        <button
          ...
          className={`px-5 py-2.5 rounded-xl text-sm font-semibold transition-all active:scale-95 ${
            busy
              ...
              : 'bg-gradient-to-r from-emerald-600 to-teal-600 text-white shadow-lg shadow-emerald-900/40 hover:shadow-emerald-800/50 hover:brightness-110'
          }`}
        >
      ...
      {isDeployed && (
        <div className="mt-4 relative">
          <pre className="text-[10.5px] font-mono text-slate-500 bg-black/30 rounded-lg p-3 overflow-x-auto leading-relaxed">
            {curl}
          </pre>
          <button
            onClick={() => {
            ...
            className="absolute top-2 right-2 text-[10px] text-slate-500 hover:text-slate-300 bg-slate-900/90 rounded px-2 py-1"
          >
            {copied ? '✓' : 'Copy'}
          </button>
        </div>
    ...
    <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-4">
        {schema.map((f) => (
          <div key={f.name} className="space-y-1">
            <label className="block text-[10px] font-medium text-slate-500 uppercase tracking-wider truncate">
              {f.name}
            </label>
            <input
              ...
              className="w-full bg-slate-950/80 border border-slate-700/80 rounded-lg px-3 py-2 text-sm text-slate-200 focus:outline-none focus:border-violet-500 focus:shadow-[0_0_0_3px_rgba(139,92,246,0.15)] transition-shadow"
            />
          </div>
        ))}
      </div>
      ...
      <button
        ...
        className={`w-full py-3 rounded-xl text-sm font-semibold transition-all active:scale-[0.99] ${
          predicting
            ? 'bg-violet-900/40 text-slate-500 cursor-wait'
      ...
      {/* Result */}
      {result && (
        <div className="mt-4 rounded-xl bg-gradient-to-r from-violet-500/15 to-fuchsia-500/10 border border-violet-500/30 p-5 text-center animate-[fadeSlideUp_.4s_ease]">
          <p className="text-[10px] text-violet-400 uppercase tracking-widest mb-1">
            Prediction
          </p>
...
function DriftSection({ runId, refreshKey }: { runId: string; refreshKey: number }) {
  ...
  const statusColor = {
    ...
    drifted: 'text-rose-400 bg-rose-500/10 border-rose-500/30',
  };
    ...
    <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-5">
      <div className="flex items-center justify-between mb-4">
        ...
        <button
          onClick={load}
          disabled={loading}
          className="text-[10px] text-slate-500 hover:text-slate-300 border border-slate-700 rounded-md px-2.5 py-1 transition-colors"
        >
          {loading ? '…' : '↻ Refresh'}
        </button>
      </div>
            ...
            {report.features?.map((f) => {
              ...
              return (
                <div key={f.feature} className="flex items-center gap-3">
                  <span className="text-[11px] font-mono text-slate-400 w-36 truncate text-right">
                    {f.feature}
                  </span>
                  <div className="flex-1 h-2.5 rounded-full bg-slate-800/80 overflow-hidden relative">
                    {/* threshold markers at 0.1 and 0.25 */}
                    <div className="absolute left-[20%] top-0 bottom-0 w-px bg-slate-600/60" />
                    <div className="absolute left-[50%] top-0 bottom-0 w-px bg-slate-600/60" />
                    <div
                      className={`h-full rounded-full ${barColor} transition-all duration-700`}
                      style={{ width: `${Math.max(w, 2)}%` }}
                    />
                  </div>
                  <span className="text-[11px] font-mono text-slate-500 w-14 tabular-nums">
                    {f.psi?.toFixed(3) ?? '—'}
                  </span>
                </div>
              );
    ...
    <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-5">
      ...
      <div className="space-y-1.5 max-h-64 overflow-y-auto">
        {logs.map((p) => (
          <div
            key={p.id}
            className="flex items-center gap-3 text-xs px-3 py-2 rounded-lg bg-slate-950/60 hover:bg-slate-800/40 transition-colors"
          >
            <span className="font-semibold text-violet-300 min-w-[80px]">
              {p.prediction}
            </span>
            {p.confidence != null && (
            ...
            <span className="text-slate-600 font-mono text-[10px] truncate flex-1">
