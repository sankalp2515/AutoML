    ...
    <div
      className={`rounded-xl p-4 border ${
        highlight
          ...
          : 'bg-obsidian-900 border-bone/10'
      }`}
    >
      <p className="text-xs text-bone-faint mb-1">{label}</p>
      <p
        className={`text-2xl font-bold tabular-nums ${
          highlight ? 'text-gold-300' : 'text-bone'
        }`}
      >
        {value}
      </p>
...
function ScoreChart({
  ...
  return (
    <div className="mt-4">
      <p className="text-xs text-bone-faint mb-2">
  ...
  return (
    <div className="space-y-6">
      ...
      {Object.keys(metrics).length > 0 && (
        <div>
          <h4 className="text-xs font-semibold text-bone-faint uppercase tracking-wider mb-3">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
            {Object.entries(metrics).map(([k, v]) => (
              <div
                key={k}
                className="bg-obsidian-900 border border-bone/10 rounded-lg px-3 py-2.5"
              >
                <p className="text-[11px] text-bone-faint uppercase tracking-wider">{k}</p>
                <p className="text-base font-semibold text-bone tabular-nums">
      ...
      {/* ── Calibration ────────────────────────────────────────────────────── */}
      {calibration && (
        <div className="flex items-center gap-3 bg-obsidian-900 rounded-lg px-4 py-3 border border-bone/10">
          <span
            className={`text-lg ${
          ...
          <div>
            <p className="text-xs font-medium text-bone-dim">
              Calibration:{' '}
              <span className={calibration.well_calibrated ? 'text-jade-400' : 'text-amber-400'}>
            ...
            {shap_top_features.slice(0, 10).map((feat, i) => {
              ...
              return (
                <div key={feat} className="flex items-center gap-3">
                  <span className="text-xs text-bone-faint w-4 text-right flex-shrink-0">
                    {i + 1}
                  </span>
                  <div className="flex-1 flex items-center gap-2">
                    <div
                      className="h-4 rounded-sm bg-gold-600/40"
                      style={{ width: `${barW}%` }}
                    />
                    <span className="text-xs text-bone-dim font-mono truncate max-w-[200px]">
                      {feat}
                    </span>
                  </div>
                </div>
