...
function MarkdownRenderer({ text }: { text: string }) {
  ...
  return (
    <div className="prose prose-sm prose-invert max-w-none">
      <pre className="text-xs text-bone-dim whitespace-pre-wrap font-mono leading-relaxed">
        {text}
      </pre>
    </div>
  );
...
function ArtifactCard({
  ...
  return (
    <div className="bg-obsidian-900 border border-bone/10 rounded-xl overflow-hidden">
      {/* Header row */}
      <div className="flex items-center gap-3 p-4">
        <div className="text-2xl flex-shrink-0">{meta.icon}</div>
        <div className="flex-1 min-w-0">
          <p className="text-sm font-medium text-bone">{meta.label}</p>
          <p className="text-xs text-bone-faint truncate">{meta.description}</p>
          <p className="text-xs text-bone-ghost mt-0.5">{artifact.size_kb} KB</p>
        </div>
        <div className="flex items-center gap-2 flex-shrink-0">
          {(isText || isImage) && (
            <button
              onClick={isImage ? () => setExpanded(!expanded) : loadPreview}
              className="text-xs text-bone-dim hover:text-bone border border-bone/15 rounded-md px-3 py-1.5 transition-colors"
            >
          ...
          <a
            ...
            className="text-xs text-gold-400 hover:text-gold-300 border border-gold-700/60 hover:border-gold-600 rounded-md px-3 py-1.5 transition-colors"
          >
            Download
          </a>
        </div>
...
export default function ArtifactsPanel({
  ...
  if (artifacts.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-bone-faint space-y-2">
    ...
    <div className="space-y-6">
      ...
      {files.length > 0 && (
        <div className="space-y-3">
          <h4 className="text-xs font-semibold text-bone-faint uppercase tracking-wider">
            Output Files
          </h4>
